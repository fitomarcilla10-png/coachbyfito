...
if "plantel" not in st.session_state:
    # Base de datos simulada del plantel
...
with tab_plan:
    ...
    with col1:
        ...
        with st.container(border=True):
            ...
            tec_indiv = st.text_input("Técnica Individual", placeholder="Ej: Mecánica de tiro y finalizaciones con contacto")
            tec_equipo = st.text_input("Técnica de Equipo", placeholder="Ej: Pases de contraataque y spacing")
            tactica = st.text_input("Contenido Táctico (Eje)", placeholder="Ej: Defensa presionante individual todo el campo")
    ...
    with col2:
        ...
        if st.session_state.plan_actual:
            ...
            # Botones de Acción (Exportar y Guardar)
            b1, b2 = st.columns(2)
            with b1:
                # Función de Exportación a Texto (Fácil de imprimir o pasar a PDF/Word)
                st.download_button(
            ...
            with b2:
                # Botón preparado para la futura conexión con Google Sheets
                if st.button("☁️ Guardar en Google Sheets", use_container_width=True):
                    st.info("La conexión con Google Sheets requiere configurar las credenciales de servicio. ¡Lo haremos en el próximo paso!")
...
with tab_pizarra:
    ...
    st.caption("Dibuja los desplazamientos (línea continua) y pases (línea punteada).")
    ...
    with pizarra_col1:
        # Se incrusta un lienzo. Idealmente, aquí se puede poner una imagen de media cancha de fondo.
        canvas_result = st_canvas(
            fill_color="rgba(255, 165, 0, 0.3)",  
            stroke_width=grosor,
            stroke_color=color,
            background_color="#f0f0f0",
            # background_image=Image.open("cancha.jpg") # Si tuvieras una imagen de cancha
